...
import { Search, MapPin, Calendar, Gauge, Fuel, Settings2, X, Plus, ArrowLeft, Check, Phone, Heart } from "lucide-react";
...
function AddListingModal({ onClose, onSave }) {
  ...
  const submit = () => {
    ...
    onSave({
      id: "u" + Date.now(), brand: f.brand, model: f.model, year: Number(f.year), price: Number(f.price),
      ...
      featured: false, desc: f.desc || "Təsvir əlavə edilməyib.", phone: f.phone,
    });
    setSaved(true);
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 14, maxWidth: 520, width: "100%", maxHeight: "88vh", overflowY: "auto" }}>
        ...
        {saved ? (
          ...
          <div style={{ padding: 22, display: "grid", gap: 14 }}>
            ...
            <button onClick={submit} style={{ marginTop: 4, padding: "13px", borderRadius: 8, border: "none", background: C.yellow, color: C.asphalt, fontFamily: "Oswald", fontWeight: 600, fontSize: 15.5, cursor: "pointer" }}>
              Elanı yerləşdir
            </button>
          </div>
        )}
    ...
    <div style={{ maxWidth: 880, margin: "0 auto", padding: "22px 18px 60px" }}>
      ...
      <div style={{ marginTop: 26, background: C.asphalt, borderRadius: 12, padding: 20, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <div style={{ fontFamily: "'Space Mono', monospace", color: C.yellow, fontSize: 18, marginTop: 4 }}>{l.phone || "+994 50 123 45 67"}</div>
        </div>
